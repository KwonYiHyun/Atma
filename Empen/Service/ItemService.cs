...
namespace Empen.Service
{
    public class ItemService : IItemService
    {
        ...
        public async Task<ErrorCode> giveItemAsync(int personId, int itemId, int amount)
        {
            ...
            // EF Core를 활용하는 식으로 구현하려면 
            // SELECT와 INSERT/UPDATE를 분리해야함 (+트랜잭션을 열때 IsolationLevel을 걸기 때문에 비효율적)
            // 외부 라이브러리를 사용할 게 아니라면 RawSQL로 하는게 좋음

            //using (var transaction = await _personContext.Database.BeginTransactionAsync(System.Data.IsolationLevel.Serializable))
            //{
            //    try
            //    {
            //        var item = await _personContext.person_item
            //            .FirstOrDefaultAsync(i => i.person_id == personId && i.item_id == itemId);
            ...
            //                update_date = now
            //            };
            //            _personContext.person_item.Add(newItem);
            //        }
            ...
            //        int affectedRows = await _personContext.SaveChangesAsync();

            //        if (affectedRows == 0)
            //        {
            //            _logger.LogWarning("Affrected Row is 0 in giveItemAsync");
            ...
            //    {
            //        await transaction.RollbackAsync();
            //        _logger.LogError(ex, "Faild Upsert in giveItemAsync");
            //        return ErrorCode.TransactionFailed;
            //    }
            //}
