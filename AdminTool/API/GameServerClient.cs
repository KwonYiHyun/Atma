    ...
    public class GameServerClient
    {
        ...
        public async Task<(bool, string)> editItemAsync(MasterItemDto item)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    return (result.Result == ErrorCode.Success, result.Message);
                }
            }
            return (false, "통신 에러");
        }
        ...
        public async Task<(bool, string)> editPersonStatusAsync(PersonStatusDto status)
        {
            ...
            {
                var result = await response.Content.ReadFromJsonAsync<GameResponse<string>>();
                return result != null ? (result.Result == ErrorCode.Success, result.Message) : (false, "통신 결과 에러");
            }
            return (false, "통신 에러");
        }
        #endregion
        ...
        public async Task<(bool, string)> deletePersonItemAsync(int personItemId, DateTime updateDate)
        {
            PersonItemDeleteDto dto = new PersonItemDeleteDto { person_item_id = personItemId, update_date = updateDate };

            var response = await _httpClient.PostAsJsonAsync($"/admin/person_item/delete", dto);
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadFromJsonAsync<GameResponse<string>>();
                return result != null ? (result.Result == ErrorCode.Success, result.Message) : (false, "통신 결과 에러");
        ...
        public async Task<(bool, string)> deletePersonMailAsync(int personMailId, DateTime updateDate)
        {
            PersonMailDeleteDto dto = new PersonMailDeleteDto { person_mail_id = personMailId, update_date = updateDate };

            var response = await _httpClient.PostAsJsonAsync($"/admin/person_mail/delete", dto);
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadFromJsonAsync<GameResponse<string>>();
                return result != null ? (result.Result == ErrorCode.Success, result.Message) : (false, "통신 결과 에러");
        ...
        public async Task<(bool, string)> editPersonGachaAsync(PersonGachaDto gacha)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return (result.Result == ErrorCode.Success, result.Message ?? "성공");
            }
